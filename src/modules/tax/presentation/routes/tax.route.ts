...
export function registerTaxRoutes(app: FastifyInstance, container: Container): void {
  ...
  app.post('/admin/tax-rules', { preHandler: [admin] }, controller.create.bind(controller));
  app.put('/admin/tax-rules/:id', { preHandler: [admin] }, controller.update.bind(controller));
  app.delete('/admin/tax-rules/:id', { preHandler: [admin] }, controller.remove.bind(controller));
}
